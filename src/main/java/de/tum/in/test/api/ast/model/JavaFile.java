...
public class JavaFile {
	...
	public static JavaFile getJavaFileFromFileAt(Path pathOfFile) {
		if (javaFileMatcher.matches(pathOfFile.getFileName())) {
			try {
				...
				Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).severe("IOException in JavaFile.getJavaFileFromFileAt("
						+ pathOfFile.toAbsolutePath() + "): " + e.getMessage());
				return null;
			}
		}
		return null;
	...
	public static List<JavaFile> getJavaFilesFromFilesBelow(Path pathOfDirectory) {
		try (Stream<Path> directoryContentStream = Files.walk(pathOfDirectory)) {
			return directoryContentStream.map(JavaFile::getJavaFileFromFileAt).filter(Objects::nonNull)
					.collect(Collectors.toList());
		} catch (IOException e) {
			Logger.getLogger(Logger.GLOBAL_LOGGER_NAME).severe("IOException in JavaFile.getJavaFilesFromFilesBelow("
					+ pathOfDirectory.toAbsolutePath() + "): " + e.getMessage());
			return List.of();
		}
	}
}
