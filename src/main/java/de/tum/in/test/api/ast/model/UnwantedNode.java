...
public class UnwantedNode {
	...
	public UnwantedNode(JavaFile javaFile, String unwantedNodeName, Class<? extends Node> nodeDefinedAsUnwanted) {
		...
		this.unwantedNodePositions
				.sort(Comparator.comparingInt(NodePosition::getBeginLine).thenComparing(NodePosition::getBeginColumn));
	}
	...
	public static String getFormatedFileString(Path filePath, Map<Path, List<UnwantedNode>> unwantedNodes) {
		return unwantedNodes.get(filePath).stream().map(UnwantedNode::getFormatedUnwantedNodeString)
				.collect(Collectors.joining("\n", " - In " + filePath + ":\n", ""));
	}
	...
	public static Optional<String> getMessageForUnwantedNodesForFileAt(Path pathOfJavaFile,
		...
		return Optional.of(unwantedNodes.keySet().stream()
				.map(filePath -> getFormatedFileString(filePath, unwantedNodes)).reduce(String::concat).orElse(""));
	}
	...
	public static Optional<String> getMessageForUnwantedNodesForAllFilesBelow(Path pathOfDirectory,
			Map<String, Class<? extends Node>> nodeNameUnwantedNodeMap) {
		return JavaFile.getJavaFilesFromFilesBelow(pathOfDirectory).stream()
