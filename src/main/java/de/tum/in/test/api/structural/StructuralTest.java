 ...
 *          <br>
 *          <br>
 *          1) {@link ClassTest} checks the class itself, i.e. if a specified superclass is extended and if specified
 *          interfaces and annotations are implemented<br>
 *          2) {@link ConstructorTest} checks declared constructors their including access modifiers, annotations and
 *          parameters<br>
 *          3) {@link MethodTest} checks the declared methods including their access modifiers, annotations, parameters
 *          and return type<br>
 *          4) {@link AttributeTest} checks the declared attributes including their access modifiers, annotations and
 *          types and the declared enum values of an enum<br>
 *          <br>
 *          <br>
 *          All these elements are tests based on the test.json that specifies the structural oracle, i.e. how the
 *          solution has to look like in terms
 *          of structural elements. Note: the file test.json can be automatically generated in Artemis based on the diff
 *          between template and solution repo.
 *          However, the file test.json needs to be manually adapted afterwards because not all cases can be identified
 *          properly in an automatic manner.
 *          <br>
 *          <br>
 *          To deactivate a check, simply remove the specified elements in the test.json file.
 *          If no constructors should be tested for correctness, remove {@link ConstructorTest}, otherwise one test will
 *          fail (limitation of JUnit).
 *          If no methods should be tested for correctness, remove {@link MethodTest}, otherwise one test will fail
 *          (limitation of JUnit)
 *          If no attributes and no enums should be tested for correctness, remove {@link AttributeTest}, otherwise one
